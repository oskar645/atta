...
const walletService = new wallet_service_1.WalletService(prisma);
const ADMIN_PHONES = ['79288888645', '79306939954'];
...
async function main() {
    for (const rawPhone of ADMIN_PHONES) {
        ...
        const result = await walletService.accrueManualBonusIfNeeded(lookup.userId, {
            amount: BONUS_AMOUNT,
        ...
        if (result.applied) {
            console.log(`Начислено: ${BONUS_AMOUNT}. Баланс: ${result.wallet.bonusBalance}.`);
            continue;
        }
...
//# sourceMappingURL=grant-admin-test-bonus.js.map