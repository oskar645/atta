...
exports.resolveReferralUserId = exports.buildReferralCode = void 0;
const crypto_1 = require("crypto");
const env_1 = require("../config/env");
const REFERRAL_ALGORITHM = 'aes-256-gcm';
const REFERRAL_IV_LENGTH = 12;
const REFERRAL_TAG_LENGTH = 16;
const referralKey = (0, crypto_1.createHash)('sha256')
    .update(`atta-referral:${env_1.env.JWT_ACCESS_SECRET}`)
    .digest();
const buildReferralIv = (userId) => (0, crypto_1.createHash)('sha256')
    .update(`atta-referral-iv:${userId}:${env_1.env.JWT_ACCESS_SECRET}`)
    .digest()
...
const buildReferralCode = (userId) => {
    ...
    const cipher = (0, crypto_1.createCipheriv)(REFERRAL_ALGORITHM, referralKey, iv);
...
const resolveReferralUserId = (referralCode) => {
    ...
    try {
        ...
        const tag = payload.subarray(REFERRAL_IV_LENGTH, REFERRAL_IV_LENGTH + REFERRAL_TAG_LENGTH);
        const encrypted = payload.subarray(REFERRAL_IV_LENGTH + REFERRAL_TAG_LENGTH);
        const decipher = (0, crypto_1.createDecipheriv)(REFERRAL_ALGORITHM, referralKey, iv);
...
exports.resolveReferralUserId = resolveReferralUserId;
