...
let AuthService = class AuthService {
    constructor(prisma, jwtService, storageService, walletService) {
        this.prisma = prisma;
    ...
    async signupPhone(payload) {
        ...
        const userWithAdmin = await this.ensureAdminBootstrapForPhone(user.id, normalizedPhone);
    ...
    async loginPhone(payload) {
        ...
        if (user.blockedAt) {
            throw this.createUnauthorizedError('ACCOUNT_DISABLED', 'Аккаунт временно недоступен');
        }
    ...
    async resetPasswordPhone(payload) {
        ...
        if (!user || user.deletedAt || user.status === client_1.UserStatus.DELETED) {
            throw new common_1.NotFoundException('User with this phone was not found');
        }
        await this.prisma.user.update({
            where: {
                id: user.id,
    ...
    async getMe(authUser) {
        ...
        const user = await this.findActiveUserById(authUser.userId);
    ...
    async refresh(payload) {
        ...
        const tokens = await this.buildAuthTokens(session.user, session.id, session.user.adminProfile);
        ...
        return this.buildAuthResponse(session.user, tokens);
    }
    ...
    async ensureAdminBootstrapForPhone(userId, phone) {
