...
let MediaController = class MediaController {
    ...
    uploadListingPhoto(authUser, listingId, request, file) {
        ...
        const parsedSortOrder = typeof rawSortOrder === 'string' && rawSortOrder.trim().length > 0
            ...
            : typeof rawSortOrder === 'number'
                ? rawSortOrder
                : undefined;
        return this.listingsService.uploadPhoto(authUser, listingId, this.requireImage(file, 5 * 1024 * 1024), Number.isFinite(parsedSortOrder) ? parsedSortOrder : undefined);
    }
    ...
    requireImage(file, maxSizeBytes) {
        ...
        if (!allowed.has(mime)) {
            throw new common_1.BadRequestException(`Unsupported image mime type: ${mime}`);
        }
